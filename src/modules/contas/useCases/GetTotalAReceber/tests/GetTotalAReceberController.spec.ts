...
describe("CONTAS - Get Total a Receber Controller", () => {
    ...
    it("Deve ser capaz de calcular o total a receber de todas as contas ativas", async () => {
        const contas = await request(app)
            .get("/contas/gettotalareceber")
            .send({ ativo: true })
            .set({ Authorization: `Bearer ${lojistaToken}` });

        expect(contas.status).toBe(200);
        expect(contas.body).toBe("540.00");
    });

    it("Deve ser capaz de calcular o total a receber de todas as contas ativas inadimplentes", async () => {
        const contas = await request(app)
            .get("/contas/gettotalareceber")
            .send({ ativo: true, inadimplentes: true })
            .set({ Authorization: `Bearer ${lojistaToken}` });

        expect(contas.status).toBe(200);
        expect(contas.body).toBe("80.00");
    });

    it("Deve ser capaz de calcular o total a receber de todas as contas ativas por clientes", async () => {
        const contas = await request(app)
            .get("/contas/gettotalareceber")
            .send({ ativo: true, cliente: clienteBodyC.id })
            .set({ Authorization: `Bearer ${lojistaToken}` });

        expect(contas.status).toBe(200);
        expect(contas.body).toBe("100.00");
    });

    it("Não deve ser capaz de listar as se não estiver logado", async () => {
        const response = await request(app).get("/contas/gettotalareceber");
